...
## Orbit-Plan Title Card Plates

The twelve title cards at `/one-body/orbit-plans/` use compact technical plates rather than decorative CSS orbit art.

- Shared coordinate system: every plate uses `viewBox="0 0 320 190"`. The visible drafting frame is the complete viewBox. Labels reserve the top `24` units; primary geometry stays inside `x=24..296`, `y=28..172`.
- Shared drawing language: reference paths are thin and dotted; current paths are cyan `#5bd7eb`; transfer paths are amber `#f5b447`; target planes and special-result geometry are hot pink `#ff18b0`; completed target circles may use green `#8ce66f`; all maneuver vectors use navball green `#4cff4c`; craft are orange `#ff704d` with cream `#ffd7cc` outlines.
- Motion: every plate has one transparent `planner-tail-source` path covering about `100deg` of its active trajectory and ending at the craft. `planner-card-art.js` divides that exact path into 16 contiguous copies using the approved shared width and opacity progression. No gradients, detached streaks, or generic arrow decorations may substitute for the tail.
- Depth: projected rings are split around the body where needed. Rear arcs are drawn before the body and foreground arcs after it. Craft, nodes, burn vectors, and labels are last.
- Typography: plate labels and maneuver titles use Courier Prime; joke headlines use Space Grotesk; explanatory copy remains DM Sans. Maneuver titles are the primary card label at `clamp(0.9rem, 1.05vw, 1rem)`, while joke headlines step down to `clamp(1.2rem, 1.65vw, 1.55rem)` with `1.12` line-height. The card's interaction accent remains independent from all orbital-mechanics colors.
- Hohmann: body `(160,103,r=24)`; start circle `r=48`; target circle `r=76`; transfer ellipse `rx=62, ry=sqrt(48×76)=60.4`, centered at `(146,103)`. Burn nodes are the shared right start apsis `(208,103)` and left target apsis `(84,103)`. The active cyan tail is the lower-right quarter of the current circle, expressed as `M160 151 C186.5 151 208 129.5 208 103`; it ends at Burn 1 with an upward tangent into the upward-pointing craft. Both burn vectors are tangent and green.
- Inclination change: body `(160,103,r=24)`; current and target projected circles share the horizontal node axis and the nodes `(72,103)` and `(248,103)`. Both use `rx=88`; the current plane uses `ry=18` and the target plane uses `ry=42`, expressing inclination around the fixed node axis without rotating the nodes away from one another. The craft and northbound plane-change vector sit at the right shared node. The vector, arrowhead, and `NORMAL` label are hot pink `#ff18b0`, never navball green. The cyan tail uses the explicit lower-right current-plane quarter `M160 121 C208.6 121 248 112.94 248 103`, ending upward into the craft.
- Combined transfer and plane change: body `(160,103,r=24)`; cyan start circle `rx=48,ry=17`; amber transfer ellipse `rx=72,ry=25`; hot-pink target plane `rx=88,ry=31` rotated `-18deg` about the body so the plane change is unmistakably non-coplanar. Burn 1 sits at the front of the cyan ring `(160,120)`, with the craft and navball-green vector pointing prograde screen-right. Its tail is the exact lower-left quarter `M112 103 C112 112.4 133.5 120 160 120` and ends tangent to the craft. Burn 2 combines the transfer-tangent and out-of-plane components at the upper handoff; do not use a coplanar nested pink ring or depict Burn 1 retrograde.
- Geosynchronous: body `(160,106,r=28)`; equatorial parking ring `rx=50,ry=17`; synchronous target ring `rx=92,ry=31`. The station is tangent at `(252,106)`, with a radial longitude datum that terminates at the body surface and a one-rotation period label. Its green tail uses the exact lower-right quarter `M160 137 C210.81 137 252 123.12 252 106`, ending with the station's upward tangent.
- Resonant: body `(160,103,r=24)`; green deployment circle `r=72`; amber carrier ellipse has focus at the body, periapsis `(118,103)`, shared apoapsis `(232,103)`, geometric center `(175,103)`, `rx=57`, and `ry=sqrt(57²-15²)=55`. Five deployed satellite pips occupy the target circle at `60deg`, `120deg`, `180deg`, `240deg`, and `300deg`; the release craft at `0deg` is the sixth equally spaced vehicle. The active amber tail is the exact lower-right carrier quarter `M175 158 C206.48 158 232 133.38 232 103`, ending with the release craft's upward tangent.
- Repeat ground track: globe `(160,105,r=42)`; latitude guides are clipped to the body; the cyan reduced-cycle trace is a continuous four-cycle body-fixed track across `x=48..272`, crossing the equator every `28` units at `x=48,76,104,132,160,188,216,244,272` and reaching alternating extrema near `y=55` and `y=155`. This is the compact classic reconnaissance-satellite double-sine pattern repeated twice across the plate. The active tail uses the exact lower half-wave `M188 105 C195 155 209 155 216 105`, ending at the same-direction repeat crossing `(216,105)` with an up-right tangent; the craft rotates approximately `-82deg` to match it.
- Polar mapping: globe `(160,105,r=42)`; subdued longitude/latitude graticule is clipped to the body; one cyan polar ring uses `cx=160,cy=105,rx=28,ry=72`, with its rear half behind the globe and its right/front half redrawn over it. Cyan swaths follow successive meridian passes. The active cyan tail is the lower-right quarter `M160 177 C175.5 177 188 144.8 188 105`, ending at the right-side northbound craft with an upward tangent. No hot-pink `GAP` bracket or label appears on this overview plate.
- Retrograde: globe `(160,105,r=28)`; use one cyan retrograde ring `rx=94,ry=30`, not nested cyan and pink orbits. The orange craft sits at the foreground midpoint `(160,135)`, conforms to the orbit tangent, and points screen-left/west. Its cyan tail follows the lower-right foreground quadrant into the craft with the same leftward tangent. The navball-green insertion vector points prograde relative to that westbound craft and carries the raw prograde navball glyph beside its arrowhead. The body's amber eastward spin cue is a shallow curved arrow contained inside the planet; a flat chord is forbidden.
- Elliptical: body focus `(126,104,r=24)`; amber final ellipse `cx=166,cy=104,rx=108,ry=58`, so the physical focus offset is `40`. PE `(58,104)` and AP `(274,104)` are labeled. The active tail is the exact lower-left quarter `M166 162 C106.35 162 58 136.03 58 104`, entering the upward-pointing periapsis craft tangent to the same conic. The apoapsis craft is tangent to the same conic and the coast label stays fully inside the frame.
- Molniya: use an unambiguous focus-correct vertical teaching view. The amber ellipse is centered at `(160,92)` with `rx=56,ry=70`; its lower focus is `(160,134)`, where the body of radius `22` sits. Periapsis is `(160,162)` and apoapsis is `(160,22)`. A cyan local circular-entry arc and identical 16-segment tail run `M132 134 C132 149.46 144.54 162 160 162`, ending tangent screen-right at the periapsis craft. The green prograde vector continues screen-right from that craft. The hot-pink dwell overlay is only the smooth top arc of the same amber conic, and the apoapsis craft points screen-left tangent to it. Remove any detached bracket, rotated ellipse/body mismatch, or arrow that does not share the craft tangent.
- Bi-elliptic: body `(215,104,r=22)`; cyan start circle `r=38`; green target circle `r=62`; amber first transfer and hot-pink second transfer share the high apoapsis `(24,104)` and use the body as a common focus in the physical model. Their horizontal endpoints remain exact in the plate, while the vertical display projection is explicitly compressed to `ry=66` and `ry=76` so the extreme intermediate apoapsis stays inside the thumbnail instead of clipping the frame. Ellipse 1 joins `(253,104)` to `(24,104)` at `cx=138.5,rx=114.5`; ellipse 2 joins `(24,104)` to `(277,104)` at `cx=150.5,rx=126.5`. Burn nodes are `(253,104)`, `(24,104)`, and `(277,104)`. Three green tangent burn vectors establish the sequence. The active cyan tail is the exact lower-right start-circle quarter `M215 142 C236 142 253 125 253 104`, ending with an upward tangent into the Burn 1 craft.
- Launch-site assist: globe `(160,106,r=40)`; amber latitude ring is projected at `y=116`. The green first reachable plane and hot-pink target plane are equal projected rings `rx=96,ry=23`, rotated `+17deg` and `-17deg` around the body. They share visible nodes near `(98,106)` and `(222,106)`. The active green-plane tail is the exact `100deg` ellipse arc `M82.27 70.87 A96 23 17 0 1 222.06 106.07`, ending at the orange craft aligned to the incoming tangent, approximately `28deg`. The hot-pink dogleg correction is anchored at the craft and points screen-up, representing the difference between the incoming down-right and target up-right tangent components; it must not point away from the body like a radial-out burn. The cyan surface-spin cue is a shallow eastward curved arrow wholly inside the planet, not a flat chord.

Forbidden across title plates: arbitrary ovals, craft that point radially, burn arrows that use orbit colors, conics that miss their stated nodes, ellipses centered on a focus body, target paths crossing through solid bodies without depth treatment, generic glow effects, decorative orthogonal lines, or allowing a card's hover accent to redefine maneuver semantics.

## One-Body Category Plates

The three category cards at `/one-body/` use the same `320×190` drafting plate, typography, palette, craft geometry, navball-green burn vectors, and generated 16-segment orbital tail as the orbit-plan title cards.

- Orbit Plans: body `(160,105,r=25)`; cyan current plane `rx=56,ry=20`; green altitude target `rx=92,ry=34`; hot-pink plane target `rx=82,ry=30`, rotated `-18deg`. The current-plane tail runs from the left node to the foreground midpoint `M104 105 C104 116 129 125 160 125`, ending tangent screen-right at the craft. The prograde burn continues along that tangent.
- Rendezvous: body `(160,105,r=25)`; green target orbit `rx=92,ry=34`; amber lower phasing orbit `rx=62,ry=23`. The chaser occupies the phasing orbit foreground midpoint `(160,128)` and travels screen-right; the target occupies the target orbit right node `(252,105)` and travels north. Phase spokes establish the `90deg` lead without pretending they are trajectories. The amber tail ends at the chaser along `M98 105 C98 117.7 125.8 128 160 128`.
- Satellite Transfer: host `(115,105,r=22)`; moon `(245,105,r=14)`; cyan parking orbit `r=34`; projected moon orbit `rx=130,ry=48`; moon SOI `r=28`. The focus-correct host-body transfer ellipse has endpoints `(81,105)` and `(217,105)`, center `(149,105)`, `rx=68`, `ry=58.9`, and host focus `(115,105)`. It terminates at the moon SOI rather than the moon. The hot-pink arrival segment leaves the transfer with a continuous downward tangent and bends beneath the moon into capture. The cyan parking-orbit tail ends at the departure craft with an upward tangent, and the ejection burn continues north.
